...
export default function MerchantPresentation() {
  ...
  const profile = useMemo(() => {
    ...
    if (dashRow) {
      ...
      mcaStatus = dashRow[39] ? dashRow[39].toString().trim() : ''; 
      mcaDate = dashRow[40] ? dashRow[40].toString().trim() : '';   
  ...
  return (
    <div className="bg-[#F7F9FA] min-h-full space-y-4 sm:space-y-6 -mx-2 sm:mx-0 select-none [&_*]:outline-none pb-12">
        ...
        <div className="space-y-4 sm:space-y-6 animate-fadeIn">
            ...
            <div className="bg-white p-5 sm:p-6 rounded-2xl sm:rounded-3xl shadow-sm border border-slate-100 md:col-span-2 flex flex-col justify-between">
              <div>
                <div className="flex items-center gap-2 group flex-wrap mb-1.5">
                  <h3 className="text-xl sm:text-3xl font-black text-slate-900 leading-tight">{profile.cleanName}</h3>
                  <button onClick={() => handleCopyShortcut(profile.cleanName, 'name')} className="p-1.5 bg-slate-50 hover:bg-slate-100 text-slate-400 hover:text-[#00B14F] rounded-lg transition-all" title="Copy Nama">
                ...
                <div className="flex flex-wrap gap-2.5 mb-5 mt-2.5">
                  ...
                  <span className="px-3 py-1 bg-emerald-50 border border-emerald-100 text-emerald-600 font-bold text-[10px] sm:text-xs rounded-lg flex items-center gap-1.5 uppercase tracking-wider">
                    <UserCircle size={14}/> AM: {profile.shortAmName}
                  </span>
                  <span className="px-3 py-1 bg-blue-50 border border-blue-100 text-blue-600 font-bold text-[10px] sm:text-xs rounded-lg flex items-center gap-1.5 uppercase tracking-wider">
                    <MapPin size={14}/> {profile.city}
                  </span>
                  <span className="px-3 py-1 bg-purple-50 border border-purple-100 text-purple-600 font-bold text-[10px] sm:text-xs rounded-lg flex items-center gap-1.5 uppercase tracking-wider">
                    <Percent size={14}/> Comm: {profile.baseComm}
                  </span>
                  <span className="px-3 py-1 bg-pink-50 border border-pink-100 text-pink-600 font-bold text-[10px] sm:text-xs rounded-lg flex items-center gap-1.5 uppercase tracking-wider">
                    <Image size={14}/> Foto: {profile.photoPenetration}
                  </span>
                </div>
              </div>
              ...
              <div className="bg-[#E5F7ED] p-4 rounded-2xl border border-[#00B14F]/20 flex flex-col justify-center w-full mt-2">
                 <div className="flex justify-between items-center mb-1.5">
                   ...
                   {profile.mcaStatus.toLowerCase() === 'disbursed' ? (
                     <span className="text-[10px] bg-[#00B14F] text-white px-2.5 py-1 rounded-md font-bold shadow-sm flex items-center gap-1.5 uppercase tracking-wider"><CheckCircle2 size={12}/> Cair: {profile.mcaDate}</span>
                 ...
                 <span className="font-black text-[#00B14F] text-xl sm:text-3xl tracking-tight">{formatRupiah(profile.mcaAmount)}</span>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-4 mx-2 sm:mx-0">
            ...
            <div className="bg-white p-4 rounded-2xl sm:rounded-3xl shadow-sm border border-slate-100 flex items-center gap-3">
              ...
              <div><span className="text-[9px] font-bold text-slate-400 uppercase block tracking-wider">Avg. Investasi</span><p className="text-sm sm:text-base font-black text-slate-900">{profile.avgInvestmentRate}</p></div>
            </div>
            <div className="bg-white p-4 rounded-2xl sm:rounded-3xl shadow-sm border border-slate-100 flex items-center gap-3">
              <div className="p-3 bg-blue-50 text-blue-600 rounded-xl shrink-0"><Clock size={20} /></div>
              <div><span className="text-[9px] font-bold text-slate-400 uppercase block tracking-wider">Avg. Jam Buka</span><p className="text-sm sm:text-base font-black text-slate-900">{profile.avgOnlineHours} Jam</p></div>
            </div>
      ...
      ) : <div className="bg-white p-10 rounded-3xl border border-slate-100 text-center text-slate-400 text-sm font-bold italic mx-2 sm:mx-0 shadow-sm">Silakan cari atau pilih merchant untuk memulai presentasi.</div>}
    </div>
  );
}