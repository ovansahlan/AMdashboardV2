...
// Format murni angka tanpa Rp untuk di atas batang grafik
...
export default function Dashboard() {
  ...
  // ⚡ PERBAIKAN TOTAL: Tooltip dirancang ulang agar teks sejajar (aligned), rapi, dan profesional
  const CompareTooltip = ({ active, payload, label, accentColor, useRunrate }) => {
    if (active && payload && payload.length >= 2) {
      ...
      return (
        <div className="bg-white/95 backdrop-blur-md text-slate-800 p-4 rounded-2xl shadow-[0_12px_40px_rgb(0,0,0,0.12)] border border-slate-100 min-w-[240px] outline-none flex flex-col gap-3">
          
          {/* Bagian Header: Nama Toko */}
          <div className="border-b border-slate-100 pb-2">
            <p className="font-black text-slate-900 text-[13px] truncate">{label}</p>
          </div>
          
          {/* Bagian Body: Angka-angka Sejajar */}
  ...
  return (
    <div className="p-4 sm:p-6 bg-slate-50 min-h-screen font-sans space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 text-center">
        <div className="bg-white p-4 rounded-3xl shadow-sm border border-slate-100 flex flex-col justify-center items-center hover:border-[#00B14F] transition-all cursor-default">
          <div className="flex items-center gap-1.5 text-slate-500 mb-2">
            <ShoppingCart size={14} className="text-[#00B14F]" />
            <span className="text-[10px] font-bold uppercase tracking-wider">Basket Size</span>
          </div>
          <h3 className="text-base font-black text-slate-900 truncate w-full">{kpis.basketSizeStr}</h3>
        </div>
        ...
        <div className="bg-white p-4 rounded-3xl shadow-sm border border-slate-100 flex flex-col justify-center items-center hover:border-[#00B14F] transition-all cursor-default">
          ...
          <h3 className="text-base font-black text-slate-900 truncate w-full">{kpis.investmentStr}</h3>
        </div>

        <div className="bg-white p-4 rounded-3xl shadow-sm border border-slate-100 flex flex-col justify-center items-center hover:border-[#FF7A00] transition-all cursor-default">
          <div className="flex items-center gap-1.5 text-slate-500 mb-2">
            <Megaphone size={14} className="text-[#FF7A00]" />
            <span className="text-[10px] font-bold uppercase tracking-wider">Ads Spent</span>
          </div>
          <h3 className="text-base font-black text-slate-900 truncate w-full">{kpis.adsSpentStr}</h3>
        </div>

        <div className="bg-white p-4 rounded-3xl shadow-sm border border-slate-100 flex flex-col justify-center items-center hover:border-[#00B14F] transition-all cursor-default">
          <div className="flex items-center gap-1.5 text-slate-500 mb-2">
            <Store size={14} className="text-[#00B14F]" />
            <span className="text-[10px] font-bold uppercase tracking-wider">Active Toko</span>
          </div>
          <h3 className="text-base font-black text-slate-900">{kpis.activeMerchants}</h3>
        </div>

        <div className="bg-white p-4 rounded-3xl shadow-sm border border-slate-100 flex flex-col justify-center items-center hover:border-[#00B14F] transition-all cursor-default">
          <div className="flex items-center gap-1.5 text-slate-500 mb-2">
            <Wallet size={14} className="text-[#00B14F]" />
            <span className="text-[10px] font-bold uppercase tracking-wider">MCA Disburse</span>
          </div>
          <h3 className="text-base font-black text-slate-900 truncate w-full">{kpis.mcaDisbursedStr}</h3>
        </div>

        <div className="bg-white p-4 rounded-3xl shadow-sm border border-slate-100 flex flex-col justify-center items-center hover:border-[#00B14F] transition-all cursor-default">
          <div className="flex items-center gap-1.5 text-slate-500 mb-2">
            <Award size={14} className="text-[#00B14F]" />
            <span className="text-[10px] font-bold uppercase tracking-wider">Camp Points</span>
          </div>
          <h3 className="text-base font-black text-slate-900">{kpis.campaignPointsStr}</h3>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex flex-col items-center">
          <div className="mb-6 w-full text-center border-b border-slate-100 pb-4">
            <h4 className="text-base font-black text-slate-900">Top 10 Basket Size</h4>
            <p className="text-xs text-slate-500 mt-1">Bulan Lalu vs Bulan Ini</p>
          </div>
        ...
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex flex-col items-center">
          <div className="mb-6 w-full text-center border-b border-slate-100 pb-4">
            ...
            <p className="text-xs text-slate-500 mt-1">Alokasi Biaya Promosi</p>
          </div>
          <div className="h-[350px] w-full flex justify-center">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={charts.topAds} margin={{ top: 50, right: 0, left: 0, bottom: 20 }} barGap={4}>
                ...
                <Bar dataKey="adsLM" name="Bulan Lalu" fill="#D1D5DB" radius={[4, 4, 0, 0]} barSize={16} activeBar={false} style={{ outline: 'none' }}>
                  <LabelList dataKey="adsLM" position="top" angle={-90} offset={20} formatter={formatShorthandNum} style={{ fontSize: 9, fill: '#6B7280', fontWeight: 'bold', outline: 'none' }} />
                </Bar>
                <Bar dataKey="ads" name="Bulan Ini" fill="#FF7A00" radius={[4, 4, 0, 0]} barSize={16} activeBar={false} style={{ outline: 'none' }}>
                  <LabelList dataKey="ads" position="top" angle={-90} offset={20} formatter={formatShorthandNum} style={{ fontSize: 10, fill: '#FF7A00', fontWeight: '900', outline: 'none' }} />
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex flex-col">
          <div className="mb-6 border-b border-slate-100 pb-4 flex flex-col items-center text-center">
            <div className="p-2.5 bg-[#E5F7ED] rounded-xl mb-3"><Activity size={22} className="text-[#00B14F]"/></div>
            <h4 className="text-base font-black text-slate-900">Merchant Health Status</h4>
            <p className="text-xs text-slate-500 mt-1">Performa Runrate vs Target Bulan Lalu.</p>
          </div>
          ...
          <div className="space-y-6 flex-1 flex flex-col justify-center px-4">
             ...
             <div>
                <div className="flex justify-between items-center mb-2">
                   <span className="text-[#00B14F] font-bold text-sm flex items-center gap-2"><TrendingUp size={18}/> Bertumbuh (Naik)</span>
                ...
                <div className="w-full bg-slate-100 rounded-full h-3">
                   <div className="bg-[#00B14F] h-3 rounded-full transition-all duration-500" style={{ width: `${health.growPct}%` }}></div>
                </div>
             </div>
             ...
             <div>
                <div className="flex justify-between items-center mb-2">
                   <span className="text-slate-600 font-bold text-sm flex items-center gap-2"><Minus size={18}/> Stabil (Tetap)</span>
                ...
                <div className="w-full bg-slate-100 rounded-full h-3">
                   <div className="bg-slate-400 h-3 rounded-full transition-all duration-500" style={{ width: `${health.stablePct}%` }}></div>
                </div>
             </div>
             ...
             <div>
                <div className="flex justify-between items-center mb-2">
                   <span className="text-red-500 font-bold text-sm flex items-center gap-2"><TrendingDown size={18}/> Menurun (Drop)</span>
                ...
                <div className="w-full bg-slate-100 rounded-full h-3">
                   <div className="bg-red-500 h-3 rounded-full transition-all duration-500" style={{ width: `${health.dropPct}%` }}></div>
                </div>
             </div>
          </div>
        </div>
        ...
        {/* CAMPAIGN PIE CHART */}
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex flex-col items-center">
          <div className="mb-6 w-full text-center border-b border-slate-100 pb-4 flex flex-col items-center">
            <div className="p-2.5 bg-[#FFF2E5] rounded-xl mb-3"><PieIcon size={22} className="text-[#FF7A00]"/></div>
            <h4 className="text-base font-black text-slate-900">Campaign Mix</h4>
            <p className="text-xs text-slate-500 mt-1">Distribusi partisipasi promo toko aktif.</p>
          </div>
          ...
          <div className="h-[280px] w-full flex justify-center items-center">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart margin={{ top: 0, right: 0, bottom: 0, left: 0 }}>
                ...
                <Legend verticalAlign="bottom" align="center" height={20} iconType="circle" wrapperStyle={{ fontSize: '12px', fontWeight: 'bold' }} />
              </PieChart>
            </ResponsiveContainer>
          </div>
        </div>
