...
import { GlobalFilterContext } from '../App'; // ⚡ IMPORT GLOBAL CONTEXT
...
export default function MerchantList() {
  ...
  // ⚡ AMBIL STATE DARI GLOBAL CONTEXT
  ...
  const merchants = useMemo(() => {
    ...
    data.forEach((row, index) => {
      ...
      const amName = row[2] ? row[2].toString().trim() : 'Unassigned'; // Ambil Nama AM
  ...
  const processedData = useMemo(() => {
    ...
    // ⚡ FILTER AM GLOBAL BERAKSI DI SINI
    if (selectedAm !== 'All') {
  ...
  const getSortIcon = (key) => sortConfig.key !== key ? <ArrowUpDown size={14} className="text-slate-300" /> : (sortConfig.direction === 'asc' ? <ChevronUp size={14} className="text-[#00B14F]" /> : <ChevronDown size={14} className="text-[#00B14F]" />);
  ...
  return (
    <div className="p-4 sm:p-6 bg-[#F7F9FA] min-h-full space-y-6">
      ...
      <div className="bg-white p-5 rounded-3xl shadow-sm border border-slate-100 flex flex-col sm:flex-row justify-between sm:items-center gap-4">
        ...
        {/* ⚡ DROPDOWN AM GLOBAL (Tersinkronisasi dengan Dashboard) */}
        <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 px-4 py-2.5 rounded-xl">
          ...
          >
            {amList.map((am) => (
              <option key={am} value={am}>{am === 'All' ? 'Tampilkan Semua AM' : am}</option>
            ))}
          </select>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 bg-white p-3 rounded-2xl shadow-sm border border-slate-100">
        ...
        <div className="flex items-center gap-2 bg-slate-50 border border-slate-100 px-4 py-2.5 rounded-xl">
          ...
          <span className="text-sm font-semibold text-slate-500">Status Promo:</span>
          <select value={campaignFilter} onChange={(e) => setCampaignFilter(e.target.value)} className="text-sm font-bold text-slate-800 bg-transparent outline-none cursor-pointer">
                  ...
                  <tr key={merchant.id} onClick={() => navigate(`/merchant/${merchant.mexId}`)} className="hover:bg-slate-50 transition-colors group cursor-pointer">
                    ...
                    <td className="p-4">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        {/* ⚡ Menampilkan Nama AM di Tabel */}
                        <span className="flex items-center gap-1 text-[10px] text-slate-400 font-semibold"><UserCircle size={12}/> {merchant.amName}</span>
                      </div>
                    </td>
                    ...
                    <td className="p-4"><span className={`inline-flex px-3 py-1 rounded-full text-[10px] font-bold border ${getCampaignBadge(merchant.campaignStatus)}`}>{merchant.campaignStatus}</span></td>
                  </tr>
                ))
              )}
            </tbody>
